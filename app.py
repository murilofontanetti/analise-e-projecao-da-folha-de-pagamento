...
# ============================================================
# 1. FUNÇÃO DE GERAÇÃO DO PDF OFICIAL (ABA 1)
# ============================================================
def gerar_pdf(df, mes):
    ...
    def f_moeda(val):
        numero_texto = f"{val:,.2f}"
        return numero_texto.replace(',', 'X').replace('.', ',').replace('X', '.')
    ...
    def print_subtotals():
        ...
        pdf.cell(larguras[3], 6, limpar_texto(f_moeda(subtotals['Saldo com Reserva'])), border=1, align='R', fill=True)
        pdf.cell(larguras[4], 6, limpar_texto(f_moeda(subtotals['Liquidado'])), border=1, align='R', fill=True)
        pdf.cell(larguras[5], 6, limpar_texto(f_moeda(subtotals['Projeção Salarial'])), border=1, align='R', fill=True)
        pdf.cell(larguras[6], 6, limpar_texto(f_moeda(subtotals['Provisão 13º'])), border=1, align='R', fill=True)
        pdf.cell(larguras[7], 6, limpar_texto(f_moeda(subtotals['Provisão Férias'])), border=1, align='R', fill=True)
        pdf.cell(larguras[8], 6, limpar_texto(f_moeda(subtotals['Total Projetado'])), border=1, align='R', fill=True)
        ...
        pdf.cell(larguras[9], 6, limpar_texto(f_moeda(subtotals['Saldo Final'])), border=1, align='R', fill=True)
        pdf.set_text_color(0, 0, 0)
...
# ============================================================
# 2. FUNÇÃO DE GERAÇÃO DO PDF DO SIMULADOR (ABA 2)
# ============================================================
def gerar_pdf_simulador(atividade_nome, operacao, mes_inicio, impacto_final, df_comparativo, dados_impacto, descricao_estimativa):
    ...
    # ATUALIZAÇÃO: Limpa também o emoji da opção Mista
    ...
    pdf.cell(0, 6, limpar_texto(f"R$ {f_m(impacto_final)}"), border='R,B', ln=True)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(5)
    ...
    for index, row in dados_impacto.iterrows():
        ...
        pdf.cell(0, 5, limpar_texto(f"R$ {f_m(row['Valor'])}"), border='R', ln=True, align='R')
    ...
    for index, row in df_comparativo.iterrows():
        ...
        pdf.cell(larguras[1], 6, limpar_texto(f_m(row['Saldo Dotação'])), border=1, align='R', fill=preenchimento)
        pdf.cell(larguras[2], 6, limpar_texto(f_m(row['Proj. Antes'])), border=1, align='R', fill=preenchimento)
        ...
        if row['Impacto'] > 0:
            ...
            pdf.set_text_color(0, 0, 0)
        pdf.cell(larguras[3], 6, limpar_texto(f_m(row['Impacto'])), border=1, align='R', fill=preenchimento)
        
        pdf.set_text_color(0, 0, 0)
        pdf.cell(larguras[4], 6, limpar_texto(f_m(row['Proj. Depois'])), border=1, align='R', fill=preenchimento)
        
        if row['Saldo Final Antes'] < 0:
            pdf.set_text_color(200, 0, 0)
        else:
            pdf.set_text_color(0, 0, 0)
        pdf.cell(larguras[5], 6, limpar_texto(f_m(row['Saldo Final Antes'])), border=1, align='R', fill=preenchimento)
        
        if row['Saldo Final Depois'] < 0:
            pdf.set_text_color(200, 0, 0)
        else:
            pdf.set_text_color(0, 0, 0)
        pdf.cell(larguras[6], 6, limpar_texto(f_m(row['Saldo Final Depois'])), border=1, align='R', fill=preenchimento)
...
with aba1:
    ...
    if st.session_state.df_processado is not None:
        ...
        st.download_button(label="📄 Baixar Relatório Completo em PDF", data=pdf_bytes, file_name=nome_arquivo, mime="application/pdf", type="primary")
        st.divider()
...
with aba2:
    ...
    if st.session_state.df_processado is None:
        ...
        with c1:
            ...
            # ATUALIZAÇÃO: Nova opção "Mista" inserida na interface
        ...
        if st.button("📊 Processar Simulação Modular", type="primary"):
            ...
            # ATUALIZAÇÃO: A variável 'sinal' foi removida. O sistema agora confia nos valores algébricos de cada 'sim_'
            ...
            if not dict_impactos:
                ...
                for nat in todas_naturezas:
                    ...
                    if saldo_dot > 0 or imp != 0 or proj_antes > 0:
                        dados_comp.append({
                            "Natureza Base": nat, "Saldo Dotação": saldo_dot, "Proj. Antes": proj_antes,
                            "Impacto": imp, "Proj. Depois": proj_depois, "Saldo Final Antes": final_antes, "Saldo Final Depois": final_depois
                        })
                
                df_comparativo = pd.DataFrame(dados_comp).sort_values("Natureza Base")
                
                st.divider()
                st.subheader(f"⚖️ Quadro Comparativo por Natureza — {sim_atividade_visual}")
                ...
                st.dataframe(
                    df_comparativo.style.format({
                        "Saldo Dotação": "R$ {:,.2f}", "Proj. Antes": "R$ {:,.2f}", "Impacto": "R$ {:,.2f}", 
                        "Proj. Depois": "R$ {:,.2f}", "Saldo Final Antes": "R$ {:,.2f}", "Saldo Final Depois": "R$ {:,.2f}"
                    }).map(
                        lambda x: 'color: red;' if x > 0 else ('color: green;' if x < 0 else ''), subset=['Impacto']
                    ).map(
                ...
                st.divider()
                st.markdown("### 📥 Relatório Oficial de Simulação")
                
                pdf_bytes_simulador = gerar_pdf_simulador(
                    atividade_nome=sim_atividade_visual, operacao=sim_operacao, mes_inicio=sim_mes, 
                    impacto_final=impacto_final, df_comparativo=df_comparativo, dados_impacto=df_simulacao,
                    descricao_estimativa=sim_descricao
                )
                
                st.download_button(
                    label="📄 Baixar Relatório Comparativo em PDF",
                    data=pdf_bytes_simulador,
                    file_name=f"Simulacao_Impacto_{codigo_selecionado}.pdf",
